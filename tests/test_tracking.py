...
from .utils import build_figure
...
class TestTrackingClass:
    ...
    def test_loadtxt_stdout(self):
        ...
        # Run the script.
        res = build_figure(
            dirname, "dependency_loadtxt.py", {"PGFUTILS_TRACK_FILES": "1"}
        )
        assert res.returncode == 0
    ...
    def test_loadtxt_stderr(self):
        ...
        # Run the script.
        res = build_figure(
            dirname, "dependency_loadtxt.py", {"PGFUTILS_TRACK_FILES": "2"}
        )
        assert res.returncode == 0
    ...
    def test_loadtxt_file(self):
        ...
        tfn = os.path.join(dirname, "tracking.test.results")
        res = build_figure(
            dirname, "dependency_loadtxt.py", {"PGFUTILS_TRACK_FILES": tfn}
        )
        assert res.returncode == 0
    ...
    def test_pathlib_stdout(self):
        ...
        # Run the script.
        res = build_figure(
            dirname, "dependency_pathlib.py", {"PGFUTILS_TRACK_FILES": "1"}
        )
        assert res.returncode == 0
    ...
    def test_pathlib_stderr(self):
        ...
        # Run the script.
        res = build_figure(
            dirname, "dependency_pathlib.py", {"PGFUTILS_TRACK_FILES": "2"}
        )
        assert res.returncode == 0
    ...
    def test_pathlib_file(self):
        ...
        tfn = os.path.join(dirname, "tracking.test.results")
        res = build_figure(
            dirname, "dependency_pathlib.py", {"PGFUTILS_TRACK_FILES": tfn}
        )
        assert res.returncode == 0
    ...
    def test_ignore_non_image_binary(self):
        ...
        # Run the script.
        res = build_figure(
            dirname, "simple_binary_nonimage.py", {"PGFUTILS_TRACK_FILES": "1"}
        )
        assert (
            res.returncode == 0
        ), "Running tests/sources/tracking/simple_binary_nonimage.py failed."
        assert os.path.exists(
            "tests/sources/tracking/test.npy"
        ), "Test output not written."
    ...
    def test_ignore_non_binary(self):
        ...
        # Run the script.
        res = build_figure(
            dirname, "simple_nonbinary.py", {"PGFUTILS_TRACK_FILES": "1"}
        )
        assert (
            res.returncode == 0
        ), "Running tests/sources/tracking/simple_nonbinary.py failed."
        assert os.path.exists(
            "tests/sources/tracking/test.txt"
        ), "Test output not written."
        assert len(res.stdout.strip()) == 0, "Tracking output was not empty."
        self.cleanup()
    ...
    def test_nonfile_opener(self):
        ...
        assert (
            res.returncode == 0
        ), "Running tests/sources/tracking/simple_nonfile.py failed."
        assert os.path.exists(
            "tests/sources/tracking/test_nonfile.png"
        ), "Test output not written."
        assert len(res.stdout.strip()) == 0, "Tracking output was not empty."
        self.cleanup()
    ...
    def test_fdopen(self):
        ...
        assert (
            res.returncode == 0
        ), "Running tests/sources/tracking/simple_fdopen.py failed."
        assert os.path.exists(
            "tests/sources/tracking/test_fdopen.png"
        ), "Test output not written."
        assert len(res.stdout.strip()) == 0, "Tracking output was not empty."
        self.cleanup()
    ...
    def test_nonproject(self):
        ...
        res = build_figure(
            dirname, "simple_nonproject.py", {"PGFUTILS_TRACK_FILES": "1"}
        )
        assert (
            res.returncode == 0
        ), "Running tests/sources/tracking/simple_nonproject.py failed."
    ...
    def test_netcdf4_setup(self):
        ...
        res = build_figure(
            dirname, "netcdf4_in_setup.py", {"PGFUTILS_TRACK_FILES": "1"}
        )
        assert res.returncode == 0
        actual = set(res.stdout.strip().splitlines())
        expected = {
