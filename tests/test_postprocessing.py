...
def test_fix_raster_paths():
    ...
    assert (
        res.returncode == 0
    ), "Building tests/sources/fix_raster_paths/figures/noise.pypgf failed."
    res = build_figure(dir, "speckle.py")
    assert (
        res.returncode == 0
    ), "Building tests/sources/fix_raster_paths/speckle.pypgf failed."
...
def test_tikzpicture():
    ...
    assert (
        res.returncode == 0
    ), "Building tests/sources/tikzpicture/square.pypgf failed."
...
def test_legend_parameters():
    ...
    assert (
        res.returncode == 0
    ), "Building tests/sources/legend/legend_only.pypgf failed."
